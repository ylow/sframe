...
def plot(x, y, xlabel=LABEL_DEFAULT, ylabel=LABEL_DEFAULT, title=LABEL_DEFAULT):
    ...
    * If `x` and `y` are both SArrays of string, show a categorical heat map.
    ...
    >>> x = sframe.SArray(['dog', 'cat', 'dog', 'dog', 'cat'])
    >>> y = sframe.SArray(['happy', 'grumpy', 'grumpy', 'happy', 'grumpy'])
    >>> sframe.show(x, y)
...
def scatter(x, y, xlabel=LABEL_DEFAULT, ylabel=LABEL_DEFAULT, title=LABEL_DEFAULT):
    ...
    >>> x = sframe.SArray([1,2,3,4,5])
    >>> y = x * 2
    >>> scplt = sframe.visualization.scatter(x, y)
    """
    if (
        ...
        raise ValueError(
            "sframe.visualization.scatter supports "
...
def categorical_heatmap(
    ...
    >>> x = sframe.SArray(['1','2','3','4','5'])
    >>> y = sframe.SArray(['a','b','c','d','e'])
    >>> catheat = sframe.visualization.categorical_heatmap(x, y)
    """
    if (
        not isinstance(x, tc.data_structures.sarray.SArray)
        or not isinstance(y, tc.data_structures.sarray.SArray)
        ...
        raise ValueError(
            "sframe.visualization.categorical_heatmap supports "
...
def heatmap(x, y, xlabel=LABEL_DEFAULT, ylabel=LABEL_DEFAULT, title=LABEL_DEFAULT):
    ...
    >>> x = sframe.SArray([1,2,3,4,5])
    >>> y = x * 2
    >>> heat = sframe.visualization.heatmap(x, y)
    """
    if (
        not isinstance(x, tc.data_structures.sarray.SArray)
        or not isinstance(y, tc.data_structures.sarray.SArray)
        ...
        raise ValueError(
            "sframe.visualization.heatmap supports "
...
def box_plot(x, y, xlabel=LABEL_DEFAULT, ylabel=LABEL_DEFAULT, title=LABEL_DEFAULT):
    ...
    >>> bp = sframe.visualization.box_plot(tc.SArray(['a','b','c','a','a']),tc.SArray([4.0,3.25,2.1,2.0,1.0]))
    """
    if (
        not isinstance(x, tc.data_structures.sarray.SArray)
        or not isinstance(y, tc.data_structures.sarray.SArray)
        ...
        raise ValueError(
            "sframe.visualization.box_plot supports "
            + "x as SArray of dtype str and y as SArray of dtype: int, float."
            + "\nExample: sframe.visualization.box_plot(tc.SArray(['a','b','c','a','a']),tc.SArray([4.0,3.25,2.1,2.0,1.0]))"
        )
    title = _get_title(title)
...
def columnwise_summary(sf):
    ...
    >>> x = sframe.SArray([1,2,3,4,5])
    >>> s = sframe.SArray(['a','b','c','a','a'])
    >>> sf_test = sframe.SFrame([x,x,x,x,s,s,s,x,s,x,s,s,s,x,x])
    >>> colsum = sframe.visualization.columnwise_summary(sf_test)
    """
    if not isinstance(sf, tc.data_structures.sframe.SFrame):
        raise ValueError(
            "sframe.visualization.columnwise_summary " + "supports SFrame"
        )
...
def histogram(sa, xlabel=LABEL_DEFAULT, ylabel=LABEL_DEFAULT, title=LABEL_DEFAULT):
    ...
    >>> x = sframe.SArray([1,2,3,4,5,1,1,1,1,2,2,3,2,3,1,1,1,4])
    >>> hist = sframe.visualization.histogram(x)
    """
    if not isinstance(sa, tc.data_structures.sarray.SArray) or sa.dtype not in [
        ...
        raise ValueError(
            "sframe.visualization.histogram supports "
            + "SArrays of dtypes: int, float"
        )
    title = _get_title(title)
...
def item_frequency(sa, xlabel=LABEL_DEFAULT, ylabel=LABEL_DEFAULT, title=LABEL_DEFAULT):
    ...
    >>> x = sframe.SArray(['a','ab','acd','ab','a','a','a','ab','cd'])
    >>> ifplt = sframe.visualization.item_frequency(x)
    """
    if not isinstance(sa, tc.data_structures.sarray.SArray) or sa.dtype != str:
        raise ValueError(
            "sframe.visualization.item_frequency supports " + "SArrays of dtype str"
        )
    title = _get_title(title)
