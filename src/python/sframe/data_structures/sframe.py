...
def load_sframe(filename):
    ...
    Examples
    --------
    >>> sf = sframe.SFrame({'id':[1,2,3], 'val':['A','B','C']})
    >>> sf.save('my_sframe')        # 'my_sframe' is a directory
    >>> sf_loaded = sframe.load_sframe('my_sframe')
    """
...
class SFrame(object):
    ...
    :class:`~sframe.SArray`, but SFrames are mutable in that columns can be
    ...
    * HTTP(S).
    ...
    and examples, please see the `User Guide <https://apple.github.io/sframe/docs/user
    ...
    **Parsing**
    ...
    The :py:func:`sframe.SFrame.read_csv()` is quite powerful and, can be
    ...
    **Saving**
    ...
    >>> sf.save('mysframedir')
    >>> sf2 = sframe.load_sframe('mysframedir')
    ...
    **Logical Filter**
    ...
    See :class:`~sframe.SArray` for more details on the use of the logical
    filter.
    ...
    def read_csv_with_errors(
        ...
        **kwargs
        ...
        >>> (sf, bad_lines) = sframe.SFrame.read_csv_with_errors(bad_url)
        >>> sf
        +---------+----------+--------+
        | user_id | movie_id | rating |
        +---------+----------+--------+
    ...
    def read_csv(
        ...
        **kwargs
        ...
        >>> sf = sframe.SFrame.read_csv(url, nrows=100)
        >>> sf
        Columns:
          user_id int
          movie_id  int
        ...
        >>> sf = sframe.SFrame.read_csv(url, column_type_hints=str)
        >>> sf
        Columns:
          user_id  str
          movie_id  str
        ...
        >>> sf = sframe.SFrame.read_csv(url, header=False)
        >>> sf
        Columns:
          X1  str
          X2  str
        ...
        >>> sf = sframe.SFrame.read_csv(url, na_values=['3'], column_type_hints=str)
        >>> sf
        Columns:
          user_id str
          movie_id  str
        ...
        >>> bad_url = 'https://static.turi.com/datasets/bad_csv_example.csv'
        >>> sf = sframe.SFrame.read_csv(bad_url, error_bad_lines=True)
    ...
    def from_sql(
        ...
        >>> sframe.SFrame.from_sql(conn, "SELECT * FROM foo")
        Columns:
                a       int
                b       int
        Rows: 1
    ...
    def apply(self, fn, dtype=None):
        """
        Transform each row to an :class:`~sframe.SArray` according to a
        ...
        >>> sf = sframe.SFrame({'user_id': [1, 2, 3], 'movie_id': [3, 3, 6],
                                  'rating': [4, 5, 1]})
    ...
    def random_split(self, fraction, seed=None, exact=False):
        ...
        >>> sf = sframe.SFrame({'id': range(1024)})
    ...
    def topk(self, column_name, k=10, reverse=False):
        ...
        Examples
        --------
        >>> sf = sframe.SFrame({'id': range(1000)})
        >>> sf['value'] = -sf['id']
        >>> sf.topk('id', k=3)
        +--------+--------+
        |   id   |  value |
    ...
    def select_column(self, column_name):
        """
        Get a reference to the :class:`~sframe.SArray` that corresponds with
        ...
        Examples
        --------
        >>> sf = sframe.SFrame({'user_id': [1,2,3],
    ...
    def add_column(self, data, column_name="", inplace=False):
        ...
        --------
        >>> sf = sframe.SFrame({'id': [1, 2, 3], 'val': ['A', 'B', 'C']})
        >>> sa = sframe.SArray(['cat', 'dog', 'fossa'])
    ...
    def add_columns(self, data, column_names=None, inplace=False):
        ...
        --------
        >>> sf = sframe.SFrame({'id': [1, 2, 3], 'val': ['A', 'B', 'C']})
        >>> sf2 = sframe.SFrame({'species': ['cat', 'dog', 'fossa'],
        ...                        'age': [3, 5, 9]})
    ...
    def remove_columns(self, column_names, inplace=False):
        ...
        Examples
        --------
        >>> sf = sframe.SFrame({'id': [1, 2, 3], 'val1': ['A', 'B', 'C'], 'val2' : [10, 11, 12]})
    ...
    def append(self, other):
        ...
        Examples
        --------
        >>> sf = sframe.SFrame({'id': [4, 6, 8], 'val': ['D', 'F', 'H']})
        >>> sf2 = sframe.SFrame({'id': [1, 2, 3], 'val': ['A', 'B', 'C']})
        >>> sf = sf.append(sf2)
        >>> sf
        +----+-----+
        | id | val |
    ...
    def groupby(self, key_column_names, operations, *args):
        ...
        VAR. See :mod:`~sframe.aggregate` for more detail on the aggregators.
    ...
    def join(self, right, on=None, how="inner", alter_name=None):
        ...
        Examples
        --------
        >>> animals = sframe.SFrame({'id': [1, 2, 3, 4],
        ...                           'name': ['dog', 'cat', 'sheep', 'cow']})
        >>> sounds = sframe.SFrame({'id': [1, 3, 4, 5],
    ...
    def filter_by(self, values, column_name, exclude=False):
        ...
        given ``values`` :class:`~sframe.SArray`. If ``values`` is not an
    ...
    def show(self):
        ...
        - The plot will render either inline in a Jupyter Notebook, in a web
          ...
          `sframe.visualization.set_target` (defaults to 'auto').
    ...
    def pack_columns(
        ...
        >>> sf = sframe.SFrame({'business': range(1, 5),
    ...
    def split_datetime(
        ...
        other, refer to :py:func:`sframe.SArray.split_datetime()`
    ...
    def unpack(
        ...
        >>> sf = sframe.SFrame([{'a':1,'b':2,'c':3},{'a':4,'b':5,'c':6}])
        >>> sf.unpack()
        +---+---+---+
        | a | b | c |
        +---+---+---+
    ...
    def stack(
        ...
        >>> sf = sframe.SFrame({'topic':[1,2,3,4],
        ...                       'words': [{'a':3, 'cat':2},
        ...                                 {'a':1, 'the':2},
        ...                                 {'the':1, 'dog':3},
        ...                                 {}]
    ...
    def unstack(self, column_names, new_column_name=None):
        ...
        groupby : ``unstack`` is a special version of ``groupby`` that uses the
          :mod:`~sframe.aggregate.CONCAT` aggregator
        ...
        Examples
        --------
        >>> sf = sframe.SFrame({'count':[4, 2, 1, 1, 2, None],
        ...
        >>> sf = sframe.SFrame({'friend': [2, 3, 4, 5, 6, 4, 5, 2, 3],
    ...
    def unique(self):
        ...
        Examples
        --------
        >>> sf = sframe.SFrame({'id':[1,2,3,3,4], 'value':[1,2,3,3,4]})
        >>> sf
        +----+-------+
        | id | value |
        +----+-------+
    ...
    def dropna(self, columns=None, how="any", recursive=False):
        ...
        >>> sf = sframe.SFrame({'a': [1, None, None], 'b': ['a', 'b', None]})
        >>> sf.dropna()
        +---+---+
        | a | b |
        +---+---+
    ...
    def dropna_split(self, columns=None, how="any", recursive=False):
        ...
        same functionality as :py:func:`~sframe.SFrame.dropna`, but returns a
        tuple of two SFrames.  The first item is the expected output from
        :py:func:`~sframe.SFrame.dropna`, and the second item contains all the
    ...
    def fillna(self, column_name, value):
        ...
        Examples
        --------
        >>> sf = sframe.SFrame({'a':[1, None, None],
        ...                       'b':['13.1', '17.2', None]})
        >>> sf = sf.fillna('a', 0)
        >>> sf
        +---+------+
