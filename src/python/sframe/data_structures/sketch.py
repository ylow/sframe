...
class Sketch(object):
    ...
    >>> my_sarray = sframe.SArray([1,2,3,4,5])
    >>> sketch = sframe.Sketch(my_sarray)
    >>> sketch = my_sarray.summary()

    Typically, the SArray is a column of an SFrame:

    >>> my_sframe =  sframe.SFrame({'column1': [1,2,3]})
    >>> sketch = sframe.Sketch(my_sframe['column1'])
    ...
    For numeric columns, the following information is provided exactly:
     ...
     - length (:func:`~sframe.Sketch.size`)
     - number of missing Values (:func:`~sframe.Sketch.num_missing`)
     - minimum  value (:func:`~sframe.Sketch.min`)
     - maximum value (:func:`~sframe.Sketch.max`)
     - mean (:func:`~sframe.Sketch.mean`)
     - variance (:func:`~sframe.Sketch.var`)
     - standard deviation (:func:`~sframe.Sketch.std`)

    And the following information is provided approximately:

     - number of unique values (:func:`~sframe.Sketch.num_unique`)
     - quantiles (:func:`~sframe.Sketch.quantile`)
     - frequent items (:func:`~sframe.Sketch.frequent_items`)
     - frequency count for any value (:func:`~sframe.Sketch.frequency_count`)
    ...
    And the following information is provided approximately:
     ...
     - number of unique Values (:func:`~sframe.Sketch.num_unique`)
     - frequent items (:func:`~sframe.Sketch.frequent_items`)
     - frequency count of any value (:func:`~sframe.Sketch.frequency_count`)
    ...
    summary over flattened values. Element sub sketch may be retrieved through:
     ...
     - element_summary(:func:`~sframe.Sketch.element_summary`)

    For SArray of type dict, there are sub sketches for both dict key and value.
    The sub sketch may be retrieved through:

     - dict_key_summary(:func:`~sframe.Sketch.dict_key_summary`)
     - dict_value_summary(:func:`~sframe.Sketch.dict_value_summary`)
    ...
    For example:
         ...
         >>> sa = sframe.SArray([[100,200,300,400,500], [100,200,300], [400,500]])
    ...
    def __init__(self, array=None, background=False, sub_sketch_keys=[], _proxy=None):
        ...
        background : boolean, optional
            ...
            may be queried by calling (:func:`~sframe.Sketch.sketch_ready`)
            default is False
    ...
    def element_length_summary(self):
        ...
        Examples
        --------
        >>> sa = sframe.SArray([[j for j in range(i)] for i in range(1,1000)])
    ...
    def dict_key_summary(self):
        ...
        Examples
        --------
        >>> sa = sframe.SArray([{'I':1, 'love': 2}, {'nature':3, 'beauty':4}])
    ...
    def element_sub_sketch(self, keys=None):
        ...
        Examples
        --------
        >>> sa = sframe.SArray([{'a':1, 'b':2}, {'a':4, 'd':1}])
