...
for k, v in sframe.config.get_runtime_config().items():
    log_file.write("%%s : %%s\n" %% (str(k), str(v)))
log_file.close()
...
for f in glob.glob(sframe.config.get_client_log_location() + "*"):
    try:
...
def dump_directory_structure(out=sys.stdout):
    """
    Dumps a detailed report of the sframe directory structure
