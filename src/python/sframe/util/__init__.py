...
def _get_s3_endpoint():
    ...
    return sframe.config.get_runtime_config()["TURI_S3_ENDPOINT"]
...
def _make_internal_url(url):
    ...
    For all urls:
      Expands ~ to $HOME
    For S3 urls:
      Returns the s3 URL with credentials filled in using sframe.aws.get_aws_credential().
...
def get_sframe_object_type(url):
    """
    ...
    ret = _glconnect.get_unity().get_sframe_object_type(_make_internal_url(url))
...
def _get_temp_file_location():
    ...
    >>> sframe.config.set_runtime_config('TURI_CACHE_FILE_LOCATIONS', ...)
...
def _num_available_gpus():
    ...
    from sframe.toolkits._mps_utils import has_fast_mps_support
