...
def _find_only_image_column(sframe):
    """
    Finds the only column in `sframe` with a type of sframe.Image.
    If there are zero or more than one image columns, an exception will
    be raised.
    """
    from sframe import Image
...
def _find_only_drawing_column(sframe):
    ...
    or a bitmap-based drawing column (with dtype sframe.Image)
    ...
    if (
        ...
        raise ToolkitError(
            error_message("one", "sframe.Image", "bitmap", "one", "list", "stroke")
            + corrective_action_for_user
        )
    else:
        if more_than_one_image_columns and more_than_one_stroke_columns:
            ...
            raise ToolkitError(
                error_message(
                    "no", "list", "stroke", "no", "sframe.Image", "bitmap"
                )
                + corrective_action_for_user
            )
