...
using the python import statement. Note that sframe must be imported first.
"""
...
# After which, inside sframe/__init__.py, sys.modules['sframe.extensions']
...
def _create_class_instance(class_name, _proxy):
    ...
    try:
        return _class_instance_from_name(
            "sframe.extensions." + class_name, _proxy=_proxy
        )
    except:
        pass
...
class _ExtMetaPath(object):
    ...
    def find_module(self, fullname, submodule_path=None):
        ...
        # sframe's extension library before
...
def ext_import(soname, module_subpath=""):
    ...
    Loads a sframe toolkit module (a shared library) into the
    tc.extensions namespace.

    Toolkit module created via SDK can either be directly imported,
    e.g. ``import example`` or via this function, e.g. ``sframe.ext_import("example.so")``.
    ...
    >>> sframe.ext_import('example1.so')
    ['example1.square_root']

    >>> sframe.extensions.example1.square_root(9)
    3.0
    ...
    >>> sframe.ext_import('example1.so', 'math')
    ['math.example1.square_root']
    >>> sframe.extensions.math.example1.square_root(9)
    3.0

    The module can also be imported directly, but sframe *must* be imported
    first. sframe will intercept the module loading process to load the
    toolkit.
    ...
    warnings.warn(
        "sframe.ext_import is deprecated. It will be removed in the next major release."
    )
...
def _get_toolkit_function_name_from_function(fn):
    """
    If fn is a toolkit function either imported by sframe.extensions.ext_import
