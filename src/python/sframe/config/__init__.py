...
def _init_logger():
    """
    Initialize the logging configuration for the sframe package.
...
def set_num_gpus(num_gpus):
    ...
    .. sourcecode:: python
      ...
      >> sframe.config.set_num_gpus(1)
    """
    # Currently TuriCreate only supports using one GPU. See:
    # https://github.com/apple/sframe/issues/2797
...
def get_runtime_config():
    ...
    at runtime. See :py:func:`sframe.config.set_runtime_config()` to set these
...
def set_runtime_config(name, value):
    ...
    :py:func:`sframe.config.get_runtime_config()` to get the current values for
    each variable.
